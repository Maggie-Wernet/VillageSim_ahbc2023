...
export function AddImprovement(props: { resources: Resource[], onClose: () => void, tiles: Improvement[], onSubmitForm: (improvement: Improvement) => void, giveId: number}) {
    ...
    const [houseDisable, setHouseDisable] = useState(false)
    const [fieldDisable, setFieldDisable] = useState(false)
    const [pastureDisable, setPastureDisable] = useState(false)
    const [lumberDisable, setLumberDisable] = useState(false)
    const [wellDisable, setWellDisable] = useState(false)
    ...
    function handleHouseDisable() {
            if (props.resources[1].amount < 5 || props.resources[2].amount < 1 ||props.resources[3].amount < 5 || props.resources[4].amount < 5) {
                return true
            }
    }
    ...
    function handlePastureDisable() {
            if (props.resources[0].amount < 1 || props.resources[1].amount < 2 || props.resources[4].amount < 2) {
                return true
            }
    }
    ...
    return (
        ...
        <form onSubmit={handleFormSubmit}>
            <select className="AddImprovementSelect" onChange={handleSelect}>
                ...
                <option disabled={handleHouseDisable()} value={"House"}>House</option>
                <option disabled={handleFieldDisable()} value={"Field"}>Field</option>
                <option disabled={handlePastureDisable()} value={"Pasture"}>Pasture</option>
                <option disabled={handleLumberDisable()} value={"Lumber Mill"}>Lumber Mill</option>
                <option disabled={handleWellDisable()} value={"Well"}>Well</option>
            </select>
            ...
            <div className="AddImprovementBtnContainer">
                <button type="button" className="AddImprovementBtn cancel" onClick={() => props.onClose()}>Cancel</button>
